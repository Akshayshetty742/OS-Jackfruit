...
struct monitored_entry {
    pid_t           pid;
    char            container_id[64];   /* matches monitor_ioctl.h field size */
 ...
 * Justification (README): The timer callback runs in softirq context via
 * mod_timer, but timer_callback itself is called from a kernel thread
 * context on modern kernels (hrtimer / timer workqueue), so sleeping is
 * allowed.  The ioctl path is also sleepable (process context).  A mutex
 * is therefore safe here and avoids the need to disable interrupts or
 * spin.  If this were called from a hard-IRQ handler a spinlock would be
 * required instead.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
    ...
    /*
     * list_for_each_entry_safe lets us call list_del inside the loop
     * without corrupting the iterator — 'tmp' holds the next pointer
     * before we potentially free 'entry'.
     */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        /* Validate: soft limit must be strictly less than hard limit. */
        ...
        entry->soft_warned      = 0;
        /* Safely copy the container ID; ensure NUL-termination. */
     ...
     * ============================================================== */
    {
        ...
        mutex_lock(&monitored_lock);
        list_for_each_entry_safe(entry, tmp, &monitored_list, node) {
            /* Match on both PID and container_id for exactness. */
